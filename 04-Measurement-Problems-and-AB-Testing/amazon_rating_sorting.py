...
df = pd.read_csv("hafta_4/datasets/amazon_review.csv")
df.shape
df.head()
...
df[["score_pos_neg_diff", "score_average_rating", "wilson_lower_bound"]] = df.apply(
    lambda row: pd.Series([
        score_pos_neg_diff(row["helpful_yes"], row["helpful_no"]),
        score_average_rating(row["helpful_yes"], row["helpful_no"]),
        wilson_lower_bound(row["helpful_yes"], row["helpful_no"])
    ]), axis=1
)
